...
public class Slicer : MonoBehaviour
{
    ...
    /// <returns>返回切成两半的物体，Positive、Negtivate two parts,positive part is the part same with plane normal</returns>
    public static List<GameObject> Slice(Vector3 planeNormal, Vector3 planePoint, GameObject gameObject)
    {
        ...
        positivePartVerticesID2Inndexs = new Dictionary<string, int>();//sharedVertex,要去重
        ...
        //-->变换世界坐标系下定义的平面为待切割物体的局部坐标系
        //Transform the normal so that it is aligned with the object we are slicing's transform.
        Vector3 transformedNormal = ((Vector3)(gameObject.transform.localToWorldMatrix.transpose * planeNormal)).normalized;//??公式??
        //Get the enter position relative to the object we're cutting's local transform
        Vector3 transformedStartingPoint = gameObject.transform.InverseTransformPoint(planePoint);//??公式??
        ...
        for (int subMeshIndex = 0; subMeshIndex < readyToCutGameObjectMesh.subMeshCount; subMeshIndex++)
        {
            ...
            for (int j = 0; j < subMeshTriangleIndices.Length; j += 3)
            {
                ...
                //All three vertices are on the same side,所以任取一个就知道位于哪一边
                if (vertexSides[0] == vertexSides[1] && vertexSides[0] == vertexSides[2])
                {
                    ...
                    AddSubMeshTriangleInfo(vertexSides[0], subMeshIndex, vertexIDs, vertices, normals, uv);//这个部分可能会删除vertices,所以先判断距离
                }
                else
                {
                    //--->获取截点（被平面所切的点）（！！需要重新计算UV与Normal）
                    ...
                    normalizedDistance = distance / (vertices[doubleSideVertexIndex] - vertices[singleSideVertexIndex]).magnitude;//归一化用作插值的系数
        ...
        string centerVertexPositivePartID = "-2";//positivePartVertices.Count.ToString();这些可能会和已有的重复！！！
        ...
        Material mat = gameObject.GetComponent<MeshRenderer>().material;//这里只取了第一个
        var originalCols = gameObject.GetComponents<Collider>();//删除之前的colliders
        foreach (var col in originalCols)
        {
            DestroyImmediate(col);//不能用destory（），这是下一帧才会执行
        }
        //设置mesh,material,collider
        ...
        positivePartGameObject.GetOrAddComponent<MeshRenderer>().materials = mats;//一定要设置Material
    ...
    private static void SetMesh(string whichPart)
    {
        ...
        {
            positivePartMesh = new Mesh();//重置,否则之前的数据会影响新设置的顶点数据等等
            ...
            positivePartMesh.SetUVs(0, positivePartUvs); //UV0(通道0): 主要用于基础纹理映射（diffuse texture, albedo等）,UV1(通道1): 通常用于光照贴图（lightmap）,UV2(通道2): 可用于额外的效果如细节贴图
        ...
        else
        {
            ...
            negativePartMesh.SetUVs(0, negativePartUvs); //UV0(通道0): 主要用于基础纹理映射（diffuse texture, albedo等）,UV1(通道1): 通常用于光照贴图（lightmap）,UV2(通道2): 可用于额外的效果如细节贴图
    ...
    private static void AddSubMeshTriangleInfo(bool isPositivePart, int subMeshIndex, List<string> ids, List<Vector3> vertices, List<Vector3> normals, List<Vector2> uv)
    {
        if (isPositivePart)
        {
            ...
            for (int i = 0, j = 0; i < 3; i++, j++)
            {
                ...
                {
                    verticesIndexs[i] = positivePartVerticesID2Inndexs[ids[i]];//利用已有的
                    vertices.RemoveAt(j);
                    normals.RemoveAt(j);
                    uv.RemoveAt(j);
                    j--;
                ...
                {
                    verticesIndexs[i] = newVertexIndex++;//添加新的
        ...
        else
        {
            ...
            for (int i = 0, j = 0; i < 3; i++, j++)
            {
                ...
                {
                    verticesIndexs[i] = negativePartVerticesID2Inndexs[ids[i]];//利用已有的
                    vertices.RemoveAt(j);
                    normals.RemoveAt(j);
                    uv.RemoveAt(j);
                    j--;
